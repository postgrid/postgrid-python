...
from .digital_only import DigitalOnly

__all__ = ["Cheque", "Cancellation"]
...
class Cheque(BaseModel):
    ...
    currency_code: Literal["USD", "CAD"] = FieldInfo(alias="currencyCode")
    """The currency code of the cheque.

    This can be `USD` even if drawing from a Canadian bank account and vice versa.
    Defaults to the currency of the bank account country if not otherwise specified.
    """
    ...
    deposit_ready_pdfurl: Optional[str] = FieldInfo(alias="depositReadyPDFURL", default=None)
    """
    ...
    digital_only: Optional[DigitalOnly] = FieldInfo(alias="digitalOnly", default=None)
    """The digitalOnly object contains data for digital-only cheques.

    A watermark must be provided.
    """

    envelope: Union[Literal["standard"], str, None] = None
